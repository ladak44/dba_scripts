...
#############################################################################################################
#
#  refresh_ogr_F0911.sh
#
#  ver 0.1
# 
# This script refresh data in table F0911 on ogr001q database which is used in GG replication between JDE and OPR on QA environment.
...
# 6.Disable const
# 7.Drop indexes  
# 8.Start imp F0911 to temporary table
# 9.Load from temporary table to F0911 (transformation) 
# 9.Create indexes 
...
if [ `cat ${__state_file}` == '4' ]; then
	__file_output=${__work_dir}/create_table.log
	sqlplus '/ as sysdba' @${__dir}/create_table.sql > ${__file_output}
...
# Step 8.
# Import F0911 to temporary table.
if [ `cat ${__state_file}` == '7' ]; then
	 __file_output=${__db_ogr001q_path}/imp_ogr001q_F0911.log
         if [ -e ${__file_output} ]; then
             rm -f ${__file_output}
         fi

         impdp \'/ as sysdba\' network_link=JDE001Q.EM3.ORACLECLOUD.COM TABLE_EXISTS_ACTION=truncate exclude=PROCACT_INSTANCE,constraint,index,statistics  directory=DPUMP cluster=n logfile=imp_ogr001q_F0911.log \
         tables="QADTA.F0911" REMAP_TABLE=F0911:F0911_LOAD \
         REMAP_SCHEMA=QADTA:OPRDTA \
         REMAP_TABLESPACE=QADTAT:LOAD_F0911 \
         exclude=PROCACT_INSTANCE,constraint,index,statistics \
         CONTENT=data_only
	 _check_error ${__file_output} "8"
         echo "8" > ${__state_file} 
         printf "F0911 was imported to temporary table \n"
fi

# Step 9.
# Load data
if [ `cat ${__state_file}` == '8' ]; then
	__file_output=${__work_dir}/load_table_F0911.log
	sqlplus '/ as sysdba' @${__dir}/load_table_F0911.sql > ${__file_output}
...
if [ `cat ${__state_file}` == '12' ]; then
	__file_output=${__work_dir}/drop_table.log
	sqlplus '/ as sysdba' @${__dir}/drop_table.sql > ${__file_output}
	_check_error ${__file_output} "13"
